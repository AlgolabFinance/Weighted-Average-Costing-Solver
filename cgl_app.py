...
class CGL:
    ...
    def generate_transactions_report_by_year(self, movement_tracker_df, start_year, end_year):
        ...
        while start_year <= end_year:
            report = self.generate_transactions_report(movement_tracker_df, start_year, 12, 31,
                                                       start_year, 1, 1)
            # report.to_csv(str(start_year) + '-tx-report.csv')
    ...
    def generate_unrealized_cgl_report(self, movement_tracker_df, end_year, end_month, end_day,
        ...
        while i < row_number:
            ...
            try:
                ...
                i += 1
        unrealized_report.rename(
            columns={'account': 'COIN LOCATION', 'asset': 'ASSET', 'current_balance': 'TOTAL BALANCE',
                     'value_per_unit': 'BASIS PER UNIT', 'current_value': 'BASIS BALANCE'}, inplace=True)
        unrealized_report['TOTAL USD VALUE'] = unrealized_report['TOTAL BALANCE'].astype('float64') * \
                                               unrealized_report['FMV PER UNIT'].astype('float64')
        unrealized_report['UNREALIZED GAIN (LOSS)'] = unrealized_report['TOTAL USD VALUE'].astype('float64') - \
                                                      unrealized_report['BASIS BALANCE'].astype('float64')
    ...
    def generate_unrealized_report_by_year(self, movement_tracker_df, start_year, end_year):
        ...
        years = []
        while start_year <= end_year:
            report = self.generate_unrealized_cgl_report(movement_tracker_df, start_year, 12, 31,
                                                         start_year, 1, 1)
            # report.to_csv(str(start_year) + '-unrealized-report.csv')
            reports.append(report)
            years.append(start_year)
            start_year += 1
        return reports, years
...
if __name__ == '__main__':
    ...
    if uploaded_file is not None:
        ...
        with st.spinner(text='In progress...'):
            ...
            cgl.movement_tracker.to_csv('movement_tracker.csv')
            current_year = dt.datetime.now().year
            current_month = dt.datetime.now().month
            current_day = dt.datetime.now().day
            tx_report = cgl.generate_transactions_report(cgl.movement_tracker, current_year, current_month, current_day)
            ...
            st.write(tx_report)
            cgl_report = cgl.generate_cgl_report(cgl.movement_tracker, current_year, current_month, current_day)
            ...
            with st.sidebar:
                st.markdown(
                    get_table_download_link_csv(cgl.data, output_file_prefix + 'processed_data', 'Processed Data'),
                    unsafe_allow_html=True)
                st.markdown(get_table_download_link_excel(cgl.movement_tracker, output_file_prefix + 'movement_tracker',
                                                          'Movement Tracker'),
                            unsafe_allow_html=True)
                st.markdown(
                ...
                st.markdown(get_table_download_link_excel(cgl_report, output_file_prefix + 'cgl_report', 'CGL Report'),
                            unsafe_allow_html=True)
            cgl.generate_unrealized_cgl_report(cgl.movement_tracker, current_year, current_month, current_day)

        with st.form("download"):
            start_year_input = st.text_input("Start Year")
            end_year_input = st.text_input("End Year")
            check_cgl = st.checkbox("CGL Report By Year")
            check_unrealized = st.checkbox('Unrealized CGL Report By Year')
            check_tx = st.checkbox('Transactions Report By Year')
            submitted = st.form_submit_button("Generate & Download")
            if submitted:
                start_year = int(start_year_input)
                end_year = int(end_year_input)
                if check_cgl:
                    cgl_reports, cgl_years = cgl.generate_cgl_report_by_year(cgl.movement_tracker, start_year, end_year)
                    file_name = 'cgl_by_year'
                    cgl_file = zipfile.ZipFile(file_name, 'w')
                    for (cgl_report, cgl_year) in zip(cgl_reports, cgl_years):
                        path = str(cgl_year) + '-cgl-report.csv'
